...
# Get all tags
def get_all_app_tags(db):
    app_tags = db.query(DbTag).all()
    return app_tags

# Get tag by ID
def get_app_tag_by_id(app_tag_id: int,db):
    app_tag = db.query(DbTag).filter(DbTag.id == app_tag_id).first()
    return app_tag

# Update tag by ID
def update_app_tag(app_tag_id: int, new_name: str,db):
    app_tag = db.query(DbTag).filter(DbTag.id == app_tag_id).first()
    if app_tag:
...
# Delete tag by ID
def delete_app_tag(app_tag_id: int,db):
    app_tag = db.query(DbTag).filter(DbTag.id == app_tag_id).first()
    if app_tag:
        db.delete(app_tag)
        db.commit()
